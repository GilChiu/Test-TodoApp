...
export interface ITodoItemTagsClient {
    addRemoveTags(todoItemId: number | undefined, tagIdsToAdd: number[] | null | undefined, tagIdsToRemove: number[] | null | undefined): Observable<FileResponse>;
}
...
export class TodoItemTagsClient implements ITodoItemTagsClient {
    ...
    addRemoveTags(todoItemId: number | undefined, tagIdsToAdd: number[] | null | undefined, tagIdsToRemove: number[] | null | undefined): Observable<FileResponse> {
        let url_ = this.baseUrl + "/api/TodoItemTags/AddRemoveTags?";
        if (todoItemId === null)
            throw new Error("The parameter 'todoItemId' cannot be null.");
        else if (todoItemId !== undefined)
            url_ += "TodoItemId=" + encodeURIComponent("" + todoItemId) + "&";
        if (tagIdsToAdd !== undefined && tagIdsToAdd !== null)
            tagIdsToAdd && tagIdsToAdd.forEach(item => { url_ += "TagIdsToAdd=" + encodeURIComponent("" + item) + "&"; });
        if (tagIdsToRemove !== undefined && tagIdsToRemove !== null)
            tagIdsToRemove && tagIdsToRemove.forEach(item => { url_ += "TagIdsToRemove=" + encodeURIComponent("" + item) + "&"; });
        url_ = url_.replace(/[?&]$/, "");
        ...
        return this.http.request("post", url_, options_).pipe(_observableMergeMap((response_ : any) => {
            return this.processAddRemoveTags(response_);
        })).pipe(_observableCatch((response_: any) => {
            if (response_ instanceof HttpResponseBase) {
                try {
                    return this.processAddRemoveTags(response_ as any);
                } catch (e) {
                    return _observableThrow(e) as any as Observable<FileResponse>;
                }
            } else
    ...
    protected processAddRemoveTags(response: HttpResponseBase): Observable<FileResponse> {
        const status = response.status;
        const responseBlob =
            response instanceof HttpResponse ? response.body :
            (response as any).error instanceof Blob ? (response as any).error : undefined;
